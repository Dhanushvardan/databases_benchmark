...
def main():
    ...
    try:
        ...
        start_nodes = db.get_start_nodes(limit=100)

        print(f"Using {len(start_nodes)} start nodes.")

        results = []

        for hops in [1, 2, 3]:
            print()
            print(f"Running {hops}-hop benchmark...")
            ...
            print(f"Average: {result['average_ms']:.4f} ms")
        ...
        output_file = output_dir / "neo4j_traversal.json"
